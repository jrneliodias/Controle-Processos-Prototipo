...
angulo_ref(1:nit/2)  = 50;
angulo_ref(nit/2 +1 : nit)  = 30;
...
for k = 3+d:nit
    ...
    % ----- Saída da planta
%     angulo_sensor(k) = -(a1m1+a1m2)*angulo_sensor(k-1) -(a1m1*a1m2)*angulo_sensor(k-2) ...
%                         + b0m1*pot_motor_1(k-d)  + b0m1*a1m2*pot_motor_1(k-d-1) ...
%                         - b0m2*pot_motor_2(k-d) - b0m2*a1m1*pot_motor_2(k-d-1);

    angulo_sensor(k) = -a1*angulo_sensor(k-1) -a2*angulo_sensor(k-2) + b10*pot_motor_1(k-d-1) + b11*pot_motor_1(k-2-d)...
        + b20*pot_motor_1(k-d-1) + b21*pot_motor_1(k-d-2);
