...
if mode == 1
    ...
    while true
        disp("-------- Manual Mode --------");
        disp("Enter motor power (7-15) or 'q' to quit: ");
        user_input = input("", 's');

        if any((user_input == 'q') | (user_input == 'Q'))
            disp("Exiting manual mode...");
            break;
        else

            if isnan(user_input)
                disp("Invalid input! Please enter a valid motor power (0-100).");
            else
                u0 = [user_input,'\n'];
                daqduino_write(u0,ts);
                disp("Motor powers updated!");
            end
        end
    end
elseif mode ==2

    % Automatic mode with PID controller
    disp("-------- Automatic Mode --------");
