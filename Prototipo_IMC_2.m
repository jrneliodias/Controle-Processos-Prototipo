...
if start == "y"
    arduino = serialport("COM9",250000,"Timeout",5);
    arduino.Terminator;
...
for k = 2+d:nit
    ...
    % Sinal de controle
    pot_motor_1(k) = alpha1*pot_motor_1(k-1) + ((1-alpha1)/b0m1)*erro1(k)+ a1m1*((1-alpha1)/b0m1)*erro1(k-1);
    pot_motor_2(k) = alpha2*pot_motor_2(k-1) + ((1-alpha2)/b0m2)*erro2(k)+ a1m2*((1-alpha2)/b0m2)*erro2(k-1);
